...
export default function DetailsFood() {
  ...
  const fetchData = async (foodId) => {
    try {
      setLoading(true);
      setError(null);
      const response = await fetch(`https://recipe-app-rq23.vercel.app/api/recipe/lunch/${foodId}`);
  ...
  const how_toList = parseHow_to(foods.how_to);
...
const styles = StyleSheet.create({
  scrollContainer: {
    flex: 1,
    backgroundColor: 'indigo', // Modern indigo
  },
  container: {
    alignItems: 'center',
    padding: 20,
